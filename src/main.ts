...
k.scene("game", () => {
  ...
  const roundEndController = gameManager.onStateEnter("round-end", () => {
    ...
    if (gameManager.currentRoundNb === 20 && gameManager.nbDucksShotInRound < 8) {
        k.go("special-scene")
    }
...
k.scene("special-scene", () => {
    k.add([k.rect(k.width(), k.height()), k.color(0, 0, 0)]);
    k.add([
        k.text("YOU WON\nThis is a special scene implemented by the developer.", { font: "nes", size: 8 }),
        k.anchor("center"),
        k.pos(k.center()),
  ]);
